...
- Characters not deleting when the reached the bottom of the screen, the issue was in RainDrop::fall(). while new characters were added to the top of screen, using push_front, the old ones were only removed from the list if it exceeded size. Since 'y' reset at maxDepthY without deleting off-screen characters, they 'piled' up at the bottom. I added code to check the boundary of the screen to remove characters once they exceeded the console size.
